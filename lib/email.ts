...
export async function sendEmail(options: EmailOptions): Promise<boolean> {
  try {
    ...
    await transporter.sendMail({
      from: `"progitpaie" <${process.env.SMTP_USER}>`,
      to: options.to,
...
export const emailTemplates = {
  welcome: (name: string) => ({
    subject: "Welcome to progitpaie",
    html: `<h1>Welcome ${name}!</h1><p>Your employee account has been created. You can now log in and start using progitpaie.</p>`,
  }),
