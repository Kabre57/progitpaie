...
describe("Payroll API Version Feature Flag Config", () => {
  ...
  it("retourne 'v1' par défaut si la variable n'est pas définie", () => {
    ...
    expect(getPayrollEndpoint("/my")).toBe("/api/payroll/my");
  });

  it("retourne 'v2' lorsque NEXT_PUBLIC_PAYROLL_API_VERSION=v2", () => {
    process.env.NEXT_PUBLIC_PAYROLL_API_VERSION = "v2";
    expect(getPayrollApiVersion()).toBe("v2");
    expect(getPayrollEndpoint()).toBe("/api/v2/payroll");
    expect(getPayrollEndpoint("my")).toBe("/api/v2/payroll/my");
  });
});
