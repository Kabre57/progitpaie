...
/**
 * Calcule l'ensemble de la fiche de paie et de la fiscalité
 */
export function calculatePayrollTaxes(params: TaxCalculationParams): TaxCalculationResult {
  ...
  // Indemnité de transport non imposable jusqu'à 30,000 FCFA
  ...
  const brutImposable = Math.max(0, grossSalary - Math.min(transportAllowance, 30000));

  // 1. IS (Impôt sur Salaire 1.2%)
  const isTax = Math.round(brutImposable * 0.012);
  ...
  // 3. CNPS Salarié (6.3% sur retraite, plafonné à 3 375 000 FCFA)
  const baseCNPSRetraite = Math.min(brutImposable, 3375000);
  const cnpsEmployee = Math.round(baseCNPSRetraite * 0.063);
  ...
  // CNPS Patronal: Retraite (7.7% plafonné à 3 375 000) + PF (5.75% plafonné à 75 000) + AT (3% plafonné à 75 000)
  const baseCNPSPF = Math.min(brutImposable, 75000);
  const cnpsEmployerRetraite = baseCNPSRetraite * 0.077;
  const cnpsEmployerPF = baseCNPSPF * 0.0575;
  const cnpsEmployerAT = baseCNPSPF * 0.03;
  const cnpsEmployer = Math.round(cnpsEmployerRetraite + cnpsEmployerPF + cnpsEmployerAT);

  // FDFP Patronale (TFC 1.2% + TAP 0.4% = 1.6%)
  const fdfpTax = Math.round(brutImposable * 0.016);

  // Somme des retenues salariales totales
  const totalDeductions = Math.round(itsTax + igrTax + cnpsEmployee + totalDeductionsPresences);

  // Salaire Net Final
  const netSalary = Math.max(0, grossSalary - (itsTax + igrTax + cnpsEmployee));
