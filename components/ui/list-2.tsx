  ...
  return (
    <section className={className}>
      <div className="container mx-auto px-0">
        ...
        <div className="flex flex-col rounded-2xl border border-[var(--neu-border)] bg-[var(--neu-surface)] text-[var(--neu-text)] shadow-sm overflow-hidden">
          <Separator className="bg-[var(--neu-border)]" />
          {items.map((item, index) => (
            <React.Fragment key={index}>
              <div className="flex flex-col transition-all hover:bg-[var(--neu-surface-light)]">
                <div className="grid items-center gap-4 px-4 py-4 md:px-6 md:py-6 md:grid-cols-4">
                  <div className="order-1 flex items-center gap-4 md:order-none">
                    <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-[var(--neu-surface-light)] text-[var(--neu-accent)]">
                      {item.icon}
                    </span>
                  ...
                  <div className="flex items-center gap-3 ml-auto order-3 md:order-none">
                    ...
                    {(item.link || item.onClick) && (
                      <Button 
                        ...
                        className="gap-2 h-9 px-4 rounded-lg bg-[var(--neu-surface-light)] border-[var(--neu-border)] hover:bg-[var(--neu-accent)] hover:text-white text-xs font-bold transition-all"
                      >
                        {item.link ? (
                          <a href={item.link}>
                            <span>Details</span>
