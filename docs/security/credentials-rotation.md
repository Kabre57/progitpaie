...
- Contrôle indépendant complémentaire à `2026-08-05T02:26:47Z` : le script compilé `/app/scripts/rotate-encryption-key.js --dry-run`, exécuté dans le conteneur `progitpaie-app`, a trouvé `1` utilisateur et `0` champ nécessitant une rotation. Aucune écriture n'a été effectuée. Le health check a répondu `healthy` avec une base `up` et une latence de `31 ms`.
- Contrôle de l'observation : le PID du collecteur associé à `observation-20260805T015811Z/` n'était plus actif et `app.log` était vide. Cette fenêtre est invalide comme preuve d'observation. Une sonde synthétique non authentifiée a confirmé le déploiement de la télémétrie V2 (`401` et marqueur `[PROVISIONS_V2_REQUEST]`), sans lecture ni écriture de données métier. Une nouvelle fenêtre de 168 heures doit démarrer avec un collecteur persistant ; cette sonde ne compte pas comme trafic métier.
- Rectification du contrôle de collecteur : le collecteur `observation-20260805T022901Z/` est actif sous `sudo`; sa visibilité n'était pas disponible depuis le processus de contrôle isolé. Sa fenêtre reste toutefois invalide car `docker logs --follow` a inclus des lignes antérieures à l'horodatage de départ, y compris la sonde synthétique. La prochaine fenêtre doit employer `docker logs --since <started_at>` dans un nouveau dossier, sans supprimer les artefacts précédents.
